const test = require("../models/modelotest");
        ...
        const Controllertest = {
          ObtenerUnSoloRecurso: async (req, res) => {
            ...
            const instancia = await test.findByPk(req.params.id);
          ...
          ObtenerTodosLosRecursos: async (req, res) => {
            let token = req.headers.authorization;    
            if(validadorToken(token)){ 
            const recursos_instancias = await test.findAll()
          ...
          EliminarRecurso: async (req, res) => {
            let token = req.headers.authorization;    
            if(validadorToken(token)){ 
              const resultado = await test.destroy({
                where: {
                  id: req.params.id
                }
              })
          ...
          VistaFormularioEditarRecurso: async (req, res) => {
            let token = req.headers.authorization;    
            if(validadorToken(token)){         
              let instancia = await test.findByPk(req.params.id);
         
                 let estructura_formulario = 
              `<form id="formEdit"onsubmit="return validadorGenerico()">
                <h1>Editar recurso</h1>`
        
            for (let campo in test.rawAttributes) {
              if (test.rawAttributes.hasOwnProperty(campo)) {
        
               let editable;
               campo == "id" ? editable = "disabled" : editable = "";
               let tipo;
               test.rawAttributes[campo].type.constructor.key == "INTEGER" ? tipo="number": console.log("1");
               test.rawAttributes[campo].type.constructor.key == "STRING" ? tipo = "text": console.log("2");
               test.rawAttributes[campo].type.constructor.key == "BOOLEAN" ? tipo = "checkbox": console.log("3");
               test.rawAttributes[campo].type.constructor.key == "ENUM" ? tipo = "select": console.log("4");
            ...
            estructura_formulario +=` <button type="submit" class="btn btn-danger btn-sm " onclick="AplicarEdicion('test')">Aplicar edicion</button>
            </form>`      
          ...
          ActualizarRecurso: async (req, res) => {
            ...
            let token = req.headers.authorization;    
            if(validadorToken(token)){             
              const resultado = await test.update(req.body, {
                where: {
                  id: req.params.id
                }
              })
          ...
          VistaFormularioCrearRecurso: async (req, res) => {
            ...
            estructura_formulario +=` <button type="submit" class="btn btn-danger btn-sm " onclick="AplicarAgregar('test')">Agregar</button>
            </form>`
